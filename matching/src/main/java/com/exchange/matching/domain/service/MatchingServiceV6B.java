...
@Slf4j
@Service
public class MatchingServiceV6B implements MatchingService {

    private static final String SELL_ORDER_KEY = "v6b:orders:sell:";
    private static final String BUY_ORDER_KEY = "v6b:orders:buy:";
    private static final String MATCH_STREAM_KEY = "v6b:stream:matches";
    private static final String UNMATCH_STREAM_KEY = "v6b:stream:unmatched";
    private static final String PARTIAL_MATCHED_STREAM_KEY = "v6b:stream:partialMatched";
    private static final String IDEMPOTENCY_KEY = "v6b:idempotency:orders";
    ...
    public MatchingVersion getVersion() {
        return MatchingVersion.V6B;
    }


    public MatchingServiceV6B(RedisTemplate<String, String> redisTemplate) {
        ...
        try {
            ClassPathResource resource = new ClassPathResource("scripts/matchingV6B.lua");
    ...
    private void matchingProcess(MatchingOrder order) {
        ...
        // 5. Sorted Set Key (사전식 정렬을 위해)
        String orderSortKey = createOrderSortKey(formattedTimestamp, orderId);

        // 6. 주문 실행 및 부분 체결 ID 생성
        ...
        redisTemplate.execute(
                ...
                orderSortKey                      // 정렬용 키 추가
        );
    }

    /**
     * 주문 정렬용 키 생성
     * 형식: timestamp:orderId (사전식 정렬을 위해)
     */
    private String createOrderSortKey(String timestamp, String orderId) {
        // 타임스탬프를 13자리 고정 길이로 포맷팅 (0-패딩)
        return String.format("%s:%s", timestamp, orderId);
    }
