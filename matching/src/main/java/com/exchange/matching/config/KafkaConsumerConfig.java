...
//     각 컨슈머는 이 팩토리를 통해 생성된 설정을 기반으로 작동합니다.
    @Bean
    public ConsumerFactory<String, KafkaMatchingEvent> consumerFactory() {
...
//     이 팩토리는 @KafkaListener 어노테이션이 붙은 메서드들을 실행할 컨테이너를 제공합니다.
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, KafkaMatchingEvent> kafkaListenerContainerFactory(/*ConsumerFactory<String, IdentityIntegrationCommand> consumerFactory*/) {
